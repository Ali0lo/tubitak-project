...
import { useStreakCalculator } from "@/hooks/use-streak-calculator";
import { TaskPriority } from "@/types/task";

import { AIDailySummary } from "@/components/dashboard/ai-daily-summary";
import { AIDailyBriefingWidget } from "@/components/dashboard/ai-daily-briefing-widget";
import { AIProductivityCoachWidget } from "@/components/dashboard/ai-productivity-coach-widget";
import { SmartSchedulingRecommendations } from "@/components/dashboard/smart-scheduling-recommendations";
import { SmartRemindersCard } from "@/components/dashboard/smart-reminders-card";
import { AIWeeklyReviewReport } from "@/components/dashboard/ai-weekly-review-report";
...
import { StreakCelebrationModal } from "@/components/dashboard/streak-celebration-modal";
import { useDashboardLayoutStore } from "@/stores/use-dashboard-layout-store";

// Lazy-loaded workspace tab views for performance
const TodayWorkspace = lazy(() =>
  import("@/components/dashboard/today-workspace").then((m) => ({ default: m.TodayWorkspace }))
);
const InsightsSection = lazy(() =>
  import("@/components/dashboard/insights-section").then((m) => ({ default: m.InsightsSection }))
);
...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "today" | "insights">("overview");
  ...
  const streakStats = useStreakCalculator(allTasks);
    ...
    return (
      <AppShell title="Dashboard">
        <Card className="p-8 flex justify-center items-center">
          <Spinner label="Loading workspace dashboard..." />
        </Card>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell title="Dashboard">
      <StreakCelebrationModal currentStreak={streakStats.currentStreak} />

      <div className="space-y-6">
        {/* Workspace Top Header & Navigation Bar */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-paper-line pb-4">
          <div className="flex items-center gap-1 bg-paper-tint p-1 rounded-xl border border-paper-line">
            <button
              type="button"
              onClick={() => setActiveTab("overview")}
              className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-medium transition-all ${
                activeTab === "overview"
                  ? "bg-paper text-ink shadow-2xs font-bold border border-paper-line"
            ...
            >
              <BarChart3 className="h-3.5 w-3.5 text-forest" />
          ...
          <ExportDialog
            ...
            stats={{
              ...
              completionRate: Math.round((completedToday.length / (allTasks.length || 1)) * 100),
            }}
          />
        </div>
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            {/* AI Daily Briefing & Productivity Coach */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <AIDailyBriefingWidget tasks={allTasks} meetings={allMeetings} onCompleteTask={handleQuickComplete} />
              <AIProductivityCoachWidget tasks={allTasks} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <SmartSchedulingRecommendations tasks={allTasks} meetings={allMeetings} />
              <SmartRemindersCard tasks={allTasks} meetings={allMeetings} />
            </div>

            {/* Top Interactive Cards: Streak & Next Meeting Live Countdown & Pomodoro */}
            ...
            {/* Main Grid Section: Today's Timeline, Weekday Chart & AI Weekly Summary */}
        ...
        {/* Tab 3: Insights & Analytics Workspace */}
        {activeTab === "insights" && (
          <Suspense fallback={<Skeleton className="h-96 w-full" />}>
            <InsightsSection
              tasks={allTasks}
